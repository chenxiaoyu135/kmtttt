...
//            	self.docH = docH = canW / wsize * zoomIndex * zoom;
            ...
            ctx.drawImage(img, docX, docY, docW * ratio, docH * ratio);
		};
	}
...
$(function(){
	...
	var correct = new Correct({
		...
		canH: height,
		id: 'correct',
//		img: 'https://t.sharingschool.com/upload/2018/06/21/99260a8c9bda9a4ee0a619f776644531549e3300ada594bdbd5221e5aa128c66.jpg',
		img: 'image/p2.png',
	...
	//选择移动
    $(document).on('click', '#littleHand', function(e){
        if (!correct) {
            return;
        }
        var src = $(this).find('img').attr('src');
        if (!correct.self) {
            ...
            $(this).find('img').attr('src', 'image/n_c/littleH.png');
        } else {
            correct.self = false;

            $(this).find('img').attr('src', 'image/n_c/littleHand.png');
        }
    });
	// 放大
...
//		var gcbr = document.querySelector('#'+correct.id).getBoundingClientRect();
	...
	// 对行进行正则验证
	$(document).on('change', '#questionRow',function(e){
		var val = $(this).val();
		var exp = /^[0-9]*$/gi;
		if (!exp.test(val)) {
			$(this).val('');
	...
	// 对列进行正则验证
	$(document).on('change', '#questionCol',function(e){
		var val = $(this).val();
		var exp = /^[0-9]*$/gi;
		if (!exp.test(val)) {
			$(this).val('');
	...
	$(document).on('mouseup', '.canvas-mask .canvas-mask-text .canvas-mask-true', function(e) {
		...
		var row = $('#questionRow').val();
		var col = $('#questionCol').val();
		if (num || row && col) {
			...
			var h = '<div class="card-li-img">'+
						...
						'<div class="card-li-footer">题号'+num+'，第'+row+'行'+col+'列</div>'+
					'</div>';
	...
	$(window).resize(function(e) {
		var h = $(window).height();
		var head = $('.navbar-fixed-top').height();
		if (Math.abs(H - h) > 10) {
			H = h;
			$('.fixed-wrapper').css({'max-height': (h - head - SH) + 'px'})
		}
	})
